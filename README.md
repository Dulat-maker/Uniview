...
| Хранение | Без базы данных: кеш в памяти и JSON-файлах. Предрасчитанные демо-данные лежат в `frontend/data/seed`. Личные данные пользователя хранятся в `localStorage` браузера |
| Хостинг | Vercel |
...
2. **Settings → General → Root Directory:** `frontend`. Фреймворк Next.js определится сам.
...
**Библиотеки** (`frontend/package.json`):
...
Uniview/
├─ README.md                  ← этот файл
├─ CLAUDE.md                  ← журнал разработки и решения (история проекта)
